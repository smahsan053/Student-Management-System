...
const newStudentEnrollment = async (): Promise<StudentDetails | string> => {
...
const initApp = async () => {
    ...
    while (studentStatus === "New Student") {
        ...
        if (typeof newStudent === 'string') return console.log(newStudent);
        studentStatus = await initialize()
    }
    const selectedOption = await existingStudentsProcess()
    if (typeof selectedOption === 'string') return console.log(selectedOption)
}
initApp()
